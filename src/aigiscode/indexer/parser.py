...
from aigiscode.indexer.symbols import (
...
def discover_files(config: AigisCodeConfig) -> list[Path]:
    """Discover supported source files in the project directory."""
    files, _unsupported = discover_project_files(config)
    return files


def discover_unsupported_source_files(config: AigisCodeConfig) -> dict[str, int]:
...
def index_project(config: AigisCodeConfig, store: IndexStore) -> dict:
